...
const PAYMENT_CONFIG = {
  ...
  alipay: {
    ...
    gateway:
      import.meta.env.VITE_ALIPAY_GATEWAY_URL ||
...
export class PaymentService {
  ...
  static async processAlipayPayment(
    ...
    try {
      ...
      if (!PAYMENT_CONFIG.alipay.appId) {
        throw new Error(
          "支付宝支付配置缺失，请在.env文件中配置 VITE_ALIPAY_APP_ID",
        );
      }
  ...
  private static generateAlipayMockUrl(params: Record<string, string>): string {
  ...
  // 处理微信支付
  static async processWechatPayment(
    paymentData: PaymentData,
  ): Promise<PaymentResult> {
    try {
      // 检查微信支付配置
      if (!PAYMENT_CONFIG.wechat.appId || !PAYMENT_CONFIG.wechat.mchId) {
        throw new Error(
          "微信支付配置缺失，请在.env文件中配置 VITE_WECHAT_APP_ID 和 VITE_WECHAT_MCH_ID",
        );
      }
  ...
  private static generateWechatMockQRCode(
