...
export function normalizeTool(data: unknown): Tool {
  ...
  if (typeof raw.id !== "string")
    throw new Error("Invalid tool data: id must be string");
  if (typeof raw.name !== "string")
    throw new Error("Invalid tool data: name must be string");
  ...
  const extendedTool: ToolExtended = {
    ...
    category_name:
      typeof raw.category_name === "string" ? raw.category_name : null,
    category_slug:
      typeof raw.category_slug === "string" ? raw.category_slug : null,
...
export const useToolsStore = defineStore("tools", () => {
  ...
  const featuredTools = computed(() =>
    tools.value.filter((t) => t.is_featured),
  );
  const activeTools = computed(() =>
    tools.value.filter((t) => t.status === "active"),
  );
