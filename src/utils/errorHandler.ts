...
export class ErrorHandler {
  ...
  static handleApiError(error: any): AppError {
    if (error.response) {
      ...
      switch (status) {
        case 400:
          return this.createError('BAD_REQUEST', data.message || '请求参数错误', data)
        case 401:
        ...
        case 429:
          return this.createError('RATE_LIMIT', '请求过于频繁，请稍后重试', data)
        case 500:
          return this.createError('SERVER_ERROR', '服务器内部错误', data)
        case 502:
          return this.createError('BAD_GATEWAY', '网关错误', data)
        case 503:
          return this.createError('SERVICE_UNAVAILABLE', '服务暂时不可用', data)
        default:
          return this.createError('HTTP_ERROR', `HTTP错误: ${status}`, data)
      }
    } else if (error.request) {
      // 网络错误
      return this.createError('NETWORK_ERROR', '网络连接失败，请检查网络设置', error.request)
    } else {
      // 其他错误
      return this.createError('UNKNOWN_ERROR', error.message || '未知错误', error)
    }
  }
  ...
  static handleDatabaseError(error: any): AppError {
    if (error.code) {
      switch (error.code) {
        ...
        case '23503':
          return this.createError('FOREIGN_KEY_VIOLATION', '关联数据不存在', error)
        case '23514':
          return this.createError('CHECK_VIOLATION', '数据验证失败', error)
        default:
          return this.createError('DATABASE_ERROR', error.message || '数据库操作失败', error)
      }
    }
  ...
  static handleValidationError(errors: Record<string, string[]>): AppError {
    ...
    return this.createError('VALIDATION_ERROR', `验证失败: ${messages}`, errors)
  }
