...
export const runFullConnectionTest = async () => {
  ...
  if (results.connection.success) {
    console.log("✅ 基础连接: 通过");
    console.log(
      `   - 工具数量: ${results.connection.details?.toolsCount || 0}`,
    );
    console.log(
      `   - 用户数量: ${results.connection.details?.usersCount || 0}`,
    );
  } else {
    console.log("❌ 基础连接: 失败");
  ...
  if (results.permissions.success) {
    console.log("✅ 权限测试: 通过");
    console.log(
      `   - 工具管理: ${results.permissions.details?.canManageTools ? "✅" : "❌"}`,
    );
    console.log(
      `   - 用户管理: ${results.permissions.details?.canManageUsers ? "✅" : "❌"}`,
    );
  } else {
    console.log("❌ 权限测试: 失败");
  ...
  const overallSuccess =
    results.connection.success && results.permissions.success;
  console.log("====================");
  console.log(
    `${overallSuccess ? "🎉" : "⚠️"} 总体结果: ${overallSuccess ? "通过" : "部分失败"}`,
  );
