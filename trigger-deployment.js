...
async function triggerDeployment() {
  try {
    ...
    const deployWorkflows = workflows.workflows.filter(workflow => 
    ...
    // 选择 Supabase 部署工作流
    const supabaseWorkflow = deployWorkflows.find(w => 
      w.name.includes('Supabase') || w.path.includes('supabase-deploy')
    )
    ...
    const { data: dispatch } = await octokit.rest.actions.createWorkflowDispatch({
      owner: REPO_OWNER,
      repo: REPO_NAME,
    ...
    console.log(`   工作流详情: https://github.com/${REPO_OWNER}/${REPO_NAME}/actions/workflows/${supabaseWorkflow.id}`)
    console.log('')
    ...
    if (runs.workflow_runs.length > 0) {
      ...
      console.log(`   开始时间: ${new Date(latestRun.created_at).toLocaleString('zh-CN')}`)
