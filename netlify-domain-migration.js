...
class NetlifyDomainMigrator {
  ...
  async findSiteByDomain(domain) {
    try {
      ...
      const site = sites.find(
        s => s.custom_domain === domain || (s.domain_aliases && s.domain_aliases.includes(domain))
      )
      return site
    } catch (error) {
  ...
  async updateSiteDomain(siteId, customDomain, domainAliases = []) {
    try {
      ...
      const updatedSite = await this.makeRequest('PATCH', `/sites/${siteId}`, updateData)
      return updatedSite
    } catch (error) {
  ...
  async migrateDomain(domain, fromSiteId, toSiteId) {
    try {
      ...
      console.log(`\n🎉 域名迁移完成！`)
      console.log(`域名 ${domain} 已成功从 ${fromSite.name} 迁移到 ${toSite.name}`)
