...
class UpdateDeploymentDiagnostic {
  ...
  checkPackageManagerConsistency() {
    ...
    if (hasPackageLock && hasYarnLock) {
      this.addIssue(
        "同时存在 package-lock.json 和 yarn.lock，可能导致依赖冲突",
      );
    ...
    if (hasPackageLock && hasPnpmLock) {
      this.addIssue(
        "同时存在 package-lock.json 和 pnpm-lock.yaml，可能导致依赖冲突",
      );
    }
    ...
    workflowFiles.forEach((file) => {
      if (fs.existsSync(file)) {
        ...
        if (content.includes("yarn install") && hasPackageLock) {
          this.addWarning(
            `${file} 使用 yarn install 但项目有 package-lock.json`,
          );
        }
      }
    });
  ...
  checkNetlifyConfig() {
    ...
    if (fs.existsSync("netlify.toml")) {
      ...
      if (!content.includes("NPM_CONFIG_OPTIONAL")) {
        ...
        this.addSuggestion(
          '添加 NPM_CONFIG_OPTIONAL = "false" 到 [build.environment]',
        );
      }
    } else {
  ...
  checkCacheIssues() {
    ...
    if (fs.existsSync("node_modules")) {
      try {
        ...
        if (ageInHours > 24) {
          this.addWarning(
            `node_modules 已超过 ${Math.floor(ageInHours)} 小时未更新`,
          );
