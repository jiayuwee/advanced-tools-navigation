...
if (!supabaseUrl || !supabaseKey) {
  ...
  console.log('请检查 .env.local 文件中的 VITE_SUPABASE_URL 和 VITE_SUPABASE_ANON_KEY')
  process.exit(1)
}
...
async function verifyDeployment() {
  try {
    ...
    if (bucketsError) {
      ...
      const expectedBuckets = ['avatars', 'product-images', 'tool-icons', 'uploads']
    ...
    console.log('')
    console.log('🔗 有用的链接:')
    console.log(`📊 Supabase Dashboard: ${supabaseUrl.replace('/rest/v1', '')}/dashboard`)
    console.log(`🗄️ 数据库管理: ${supabaseUrl.replace('/rest/v1', '')}/dashboard/database/tables`)
    console.log(`🔐 认证设置: ${supabaseUrl.replace('/rest/v1', '')}/dashboard/auth/users`)
    console.log(`📁 存储管理: ${supabaseUrl.replace('/rest/v1', '')}/dashboard/storage/buckets`)
