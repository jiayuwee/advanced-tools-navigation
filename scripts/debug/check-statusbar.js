...
function checkStatusBarElement() {
  ...
  if (statusBar) {
    ...
    console.log(`   位置信息: top=${rect.top}, bottom=${rect.bottom}, height=${rect.height}`);
...
function checkOverlappingElements() {
  ...
  elementsAtPosition.forEach((el, index) => {
    ...
    console.log(`   ${index + 1}. ${el.tagName.toLowerCase()}${el.className ? '.' + el.className.split(' ').join('.') : ''} (z-index: ${styles.zIndex})`);
  });
